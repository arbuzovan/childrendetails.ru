	...
	*/
	...
	uEditor.prototype.bindFinishEvent = function() {
		...
		jQuery(document).on('click', function(e) {
			...
			if(jQuery(e.target).parents().addBack().hasClass('eip_win')) return true;
			if(jQuery(e.target).parents().addBack().hasClass('eip-ieditor-img-wrapper')) return true;
			if(jQuery(e.target).parents().addBack().hasClass('eip-ieditor-imgareaselect-wrapper')) return true;
			if(jQuery(e.target).parents().addBack().filter('#mce-modal-block').length) return true;
			if(jQuery(e.target).parents().addBack().filter('#popupLayerScreenLocker').length) return true;
			if(jQuery(e.target).parents().addBack().filter('#jGrowl').length) return true;
			if(jQuery(e.target).parents('.eip-ui-element, .ui-datepicker, .ui-datepicker-title, .ui-datepicker-header, .ui-datepicker-calendar').length) return;
			...
			for(var i = 0; i < parents.length; i++) {
				...
				if(jQuery(pNode).parents().addBack().hasClass('mceColorSplitMenu')) return true;
				if(jQuery(pNode).parents().addBack().hasClass('mceMenuItem')) return true;
				if(jQuery(pNode).parents().addBack().hasClass('mce-panel')) return true;
				if(jQuery(pNode).parents().addBack().hasClass('eip-ieditor-imgareaselect-wrapper')) return true;
			}
