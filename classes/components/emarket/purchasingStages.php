	...
	class EmarketPurchasingStages {
		...
		public function getPurchaseSteps($template = "default", $stage) {
			...
			foreach ($this->module->purchaseStages as $i => $stage) {
				...
				$steps[] = emarket::parseTemplate($template, array(
			...
			return emarket::parseTemplate($purchaseStepsTemplate, array(
				"+item" => $steps
			));
		}
		...
		public function getStage($stage) {
			...
			$hasDelivery = $umiRegistry->getVal('//modules/emarket/enable-delivery');
			$hasPayment = $umiRegistry->getVal('//modules/emarket/enable-payment');
			...
			if (!$stage || (!in_array($stage, $this->module->purchaseStages) && !in_array($stage, $this->module->availableStages))) {
		...
		public function purchase($template = 'default') {
			...
			if (sizeof($module->purchaseStages) == 2 && $stage == 'result' && !getRequest('param0')) {
				$stage = '';
			}
		...
		public function required(order $order, $step, $mode, $template) {
			...
			switch ($step) {
				case 'personal': {
					return ($mode == 'do') ? $module->savePersonalInfo($order) : $module->editPersonalInfo($order, $template);
				}
			}
		}
		...
		public function delivery(order $order, $step, $mode, $template) {
			...
			switch ($step) {
				case 'address' : {
					return ($mode == 'do') ? $module->chooseDeliveryAddress($order) : $module->renderDeliveryAddressesList($order, $template);
				}
				case 'choose': {
					return ($mode == 'do') ? $module->chooseDelivery($order) : $module->renderDeliveryList($order, $template);
				}
			}
		}
		...
		public function payment(order $order, $step, $mode, $template) {
			...
			switch (true) {
				case $step == 'choose' : {
					return ($mode == 'do') ? $module->choosePayment($order) : $module->renderPaymentsList($order, $template);
				}
				case $step == 'bonus' : {
