<?php
	...
	class EmarketUMIManagerAPI {
		...
		public function getOrder() {
			...
			if ($customer instanceof umiObject) {
				...
				foreach($customerType->getAllFields() as $field) {
					 ...
					 */
					$fieldType = $field->getFieldType();
					$value = $this->parseOrderValue($fieldType->getDataType(), $customer->getPropByName($field->getName())->getValue());
		...
		public function setOrder() {
			...
			$this->admin->setData(array('@status' => $status));
			$this->admin->doData();
		}
		...
		public function addToken() {
			...
			$request = array(
				...
				'keycode' => $umiRegistry->getVal("//settings/keycode"),
				'build' => $umiRegistry->getVal("//modules/autoupdate/system_build"),
		 ...
		 * @throws selectorException
		 */
		private function getStatusesListByObjectType($objectType, $activeId = null, $orderField = false, $orderType = false) {
			if (is_null($activeId)) {
		 ...
		 * @param customer|umiObject $customer покупатель
