<?php
	 ...
	 * Как и любому другому наследниу umiObjectProxy, товарному предложению необходим объект-источник данных (umiObject).
	 */
	class orderItem extends umiObjectProxy {
		/**
		 * @var float $price цена одного товарного наименования
		 */
		protected $price;
		/**
		 * @var float $totalOriginalPrice оригинальная цена полного количества товарного наименования
		 */
		protected $totalOriginalPrice;
		/**
		 * @var float $totalActualPrice цена полного количества товарного наименования с учетом скидки
		 */
		 ...
		 * @param umiObject $item объект наименования
		 * @param umiHierarchyElement $element связанный товар каталога
		 */
		...
		/**
		 * Возвращает оригинальную цену полного количества товарного наименования
		 * @return float
		 */
		...
		/**
		 * Возвращает цену полного количества товарного наименования с учетом скидки
		 * @return float
		 */
		...
		/**
		 * Устанавливает стоимость одного товарного наименования
		...
		/**
		 * Возвращает стоимость одного товарного наименования
		 * @return float
		 */
		...
		/**
		 * Являет ли товарное предложение цифровым
		 * @return bool
		 */
		...
		public function getItemElement($keepOrphanItem = false) {
			...
			$isFound = is_array($symlink) && count($symlink) > 0;
		...
		private static function getClassPrefixes() {
			...
			foreach ($options as $option) {
				if (substr($option, 0, $l) != $req) {
					continue;
				}

				$optionArr = explode(".", substr($option, $l));

				if (sizeof($optionArr) != 2) {
					continue;
				}
