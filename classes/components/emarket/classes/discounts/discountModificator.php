<?php
	...
	abstract class discountModificator extends umiObjectProxy {
		 ...
		 * @param umiObject $modTypeObject тип модификатора скидки
		...
		public static function create(discount $discount, umiObject $modTypeObject) {
			...
			$modificatorTypeGUID = $modTypeObject->getValue('modificator_type_guid');

			if (strlen($modificatorTypeGUID)) {
				$modificatorTypeId = umiObjectTypesCollection::getInstance()->getTypeIdByGUID($modificatorTypeGUID);
			} else {
		...
		public static function get($modObjectId, discount $discount) {
			...
			if (!$modObject instanceof umiObject) {
				return false;
			}
			
			$codeName = self::getCodeName($modObject->getValue('modificator_type_id'));

			if (!$codeName) {
				return false;
			}

			if (!self::includeModificator($codeName)) {
				return false;
			}
		...
		public static function getList($discountTypeId = false, $orderById = true) {
			...
			$key = 'discounts_modifiers_list' . intval($discountTypeId);
			$cachedModifiers = $cacheFrontend->loadData($key);

			if (is_array($cachedModifiers)) {
			...
			$modifiersList = $sel->result();
			$cacheFrontend->saveData($key, $modifiersList, 3600);
		...
		/**
		 * Конструктор
		 * @param umiObject $object объект-источник данных для модификатора скидки
		 * @throws Exception
		 */
		...
		private static function getCodeName($modTypeObjectId) {
			...
			$cache[$modTypeObjectId] = ($modTypeObject instanceof umiObject) ? trim($modTypeObject->getValue('modificator_codename')) : false;
			return $cache[$modTypeObjectId];
		}
	};
?>