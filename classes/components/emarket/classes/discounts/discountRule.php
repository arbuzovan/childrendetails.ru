<?php
	...
	abstract class discountRule extends umiObjectProxy {
		...
		public static function create(discount $discount, iUmiObject $ruleTypeObject) {
			...
			$ruleTypeId = null;
			$ruleTypeGUID = $ruleTypeObject->getValue('rule_type_guid');

			if (strlen($ruleTypeGUID)) {
		...
		public static function get($modObjectId, discount $discount) {
			...
			if ($modObject instanceof umiObject == false) {
				return false;
			}
			
			$codeName = self::getCodeName($modObject->getValue('rule_type_id'));
		...
		public static function getList($discountTypeId = false) {
			...
			$key = 'discounts_rules_list' . intval($discountTypeId);
			$cachedRules = $cacheFrontend->loadData($key);
			...
			$rules = $sel->result();
			$cacheFrontend->saveData($key, $rules, 3600);
		...
		/**
		 * Конструктор
		 * @param umiObject $object объект-источник данных для правила скидки
		 * @throws Exception
		 */
		...
		private static function getCodeName($modTypeObjectId) {
			...
			$cache[$modTypeObjectId] = ($modTypeObject instanceof umiObject) ? trim($modTypeObject->getValue('rule_codename')) : false;
			return $cache[$modTypeObjectId];
		}
	};
?>
