<?php
	...
	class russianpostDelivery extends delivery {
		 ...
		 */
		private function getEmsPrice($address, $weight, $declaredValue) {
			$umiObjects = umiObjectsCollection::getInstance();
		 ...
		 */
		private function calculateEmsPrice($fromCityName, $toCityName, $weight, $declaredValue) {
			$response = json_decode(umiRemoteFileGetter::get(self::EMS_MAX_WEIGHT_URL));
			...
			$response = json_decode(umiRemoteFileGetter::get(self::EMS_CITIES_URL));
			...
			$response = json_decode(umiRemoteFileGetter::get(self::EMS_URL . $query));
