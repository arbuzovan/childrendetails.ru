...
class ApiShipDelivery extends delivery {
	 ...
	 * @throws publicAdminException
	 */
	private function getApiShipCompanyId() {
	 ...
	 * @throws publicAdminException
	 */
	private function getCurrentApiShipUserData() {
		try {
			return $this->getRequestSender()
				->getCurrentUserData();
		} catch (Exception $e) {
	 ...
	 * @return array
	 */
	private function getDeliveryOptionsRequiredFields() {
		return [
	 ...
	 */
	private function updateOrdersStatuses(ApiShip\Orders\iEntity $order, $orderStatus, $providerNumber = null) {
	 ...
	 */
	private function updateUmiOrderDeliveryStatus($umiOrderNumber, $apiShipOrderStatus) {
	 ...
	 * @return bool была ли выполнена операция
	 */
	private function saveProviderIsConnectedFlag(ApiShip\iProvidersSettings $settings, $providerKey, $flag = true) {
