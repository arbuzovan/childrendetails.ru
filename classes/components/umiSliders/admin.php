...
class UmiSlidersAdmin implements iUmiRegistryInjector, iModulePart {
	 ...
	 * "Время задежки перед переключением слайдов по умолчанию"
	 */
	...
	public function getSliders($sliderId = null, $domainId = null, $languageId = null, $limit = null, $offset = null) {
		...
		if (!is_null($sliderId) || $this->isFilterExists()) {
	...
	public function getValidDragModes(
		iUmiCollectionItem $targetElement = null, iUmiCollectionItem $draggedElement = null
	) {
		return [
			'after',
			'before'
	...
	public function config() {
		...
		$mode = $this->getNumberedParameter(0);
	...
	public function callbackBeforeReturnEditFormData(
		array $formData, iUmiCollectionItem $entity, array $fieldsConfig
	) {
	...
	public function callbackBeforeReturnCreateFormData(
		array $formData, iUmiCollection $collection, array $fieldsConfig
	) {
	...
	public function callbackFieldValueBeforeSave(
		$fieldName, $fieldValue, iUmiCollectionItem $entity = null, iUmiCollection $collection = null
	) {
	...
	public function getSlidesList($sliderId = false) {
		if ($sliderId === false) {
			$sliderId = \UmiCms\Service::Request()
				->get()
				->get('slider_id');
		}
	...
	public function saveSlidesList(array $slidesList = []) {
		if (empty($slidesList)) {
			$slidesList = (array) \UmiCms\Service::Request()
				->post()
				->get('slide_list');
		}
	...
	public function deleteSlide($slideId = false) {
		if ($slideId === false) {
			$slideId = \UmiCms\Service::Request()
				->post()
				->get('slide_id');
		}
	...
	protected function fillBoolDefaultValues(
		iUmiCollectionItem $entity, array $createFormData, iUmiConstantMap $constants
	) {
