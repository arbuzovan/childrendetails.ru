<?php
	...
	class NewsMacros {
		...
		public function view($elementPath = '', $template = 'default') {
			...
			return $moduleClass::parseTemplate($template_block, array(
				'id' => $element->getId()
			), $element->getId());
		}
		 ...
		 * @param bool|string $elementPath путь до новости или ее ID, для которой необходимо вывести список связанных новостей
		 * @param string $template имя шаблона для TPL-шаблонизатора
		...
		public function related_links($elementPath = false, $template = 'default', $limit = 3) {
			...
			list($template_block, $template_block_empty, $template_line) = $moduleClass::loadTemplates(
				'news/' . $template,
				'related_block',
				'related_block_empty',
				'related_line'
