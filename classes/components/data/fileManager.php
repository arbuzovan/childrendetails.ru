	...
	class DataFileManager {
		...
		public function elfinder_connector($needInfo = false) {
			...
			function elfinder_full_access($attr, $path, $data, $volume) {
				...
				return strpos(basename($path), '.') === 0  ? !$readOrWrite : $readOrWrite;
			}
			...
			if (!isDemoMode() && $fileManagerDirectory = $user->getValue('filemanager_directory')) {
				...
				foreach ($directories as $directory) {
					...
					$pathNotInUserFilesPath = (strpos($directoryPath, USER_FILES_PATH ) === false);
					$pathNotInUserImagesPath = (strpos($directoryPath, USER_IMAGES_PATH ) === false);
			...
			$cwdLength = strlen(CURRENT_WORKING_DIR);
			$imagesDir = substr(USER_IMAGES_PATH, $cwdLength) . '/';
			$filesDir = substr(USER_FILES_PATH, $cwdLength) . '/';
			...
			if (count($allowedDirectories)) {
				...
				switch (true) {
					case strpos($target, self::IMAGES_HASH_PREFIX) === 0 : {
		...
		public function getfilelist() {
			...
			foreach ($directory as $file) {
				...
				if ($allowedExt !== true && !in_array(strtolower($ext), $allowedExt)) {
					continue;
				}
		...
		public function rename() {
			...
			if(
				strpos($newDir, USER_IMAGES_PATH ) === false &&
