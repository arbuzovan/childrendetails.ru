	...
	class DataAdmin {
		...
		public function type_field_add($redirectString = false) {
			...
			if ($mode == "do") {
				...
				$redirectString = ($redirectString) ? $redirectString : $fieldEditLink;
		...
		public function type_group_add($redirectString = false) {
			...
			if ($mode == "do") {
				...
				if (getRequest('noredirect')) {
					...
					$this->chooseRedirect(($redirectString ? $redirectString : ($this->module->pre_lang . '/admin/data/type_group_edit/')) . $fields_group_id . '/' . $type_id . '/');
				}
			}
...
SQL;
			...
			$is_last = ($is_last != "false") ? false : true;
		...
		public function json_move_group_after() {
			...
			$before_group_id = ($before_group_id == "false") ? true : false;
		...
		public function trash() {
			...
			$searchText = getRequest('search-all-text');
			$searchName = is_array($searchText) ? (string) array_shift($searchText) : '';
			$deleted = umiHierarchy::getInstance()->getDeletedList($total, $limit, $curr_page, $searchName);
