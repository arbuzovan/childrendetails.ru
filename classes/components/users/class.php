<?php
	...
	class users extends def_module {
		...
		public function __construct() {
			...
			if ($commonTabs) {
				$commonTabs->add('users_list_all', ['users_list']);
				$commonTabs->add('groups_list');
			}

			$umiRegistry = regedit::getInstance();
			$this->changePagesPermissionsOnEdit = (bool) $umiRegistry->getVal(
				'//modules/users/pages_permissions_changing_enabled_on_edit'
			);
			$this->changePagesPermissionsOnAdd = (bool) $umiRegistry->getVal(
				'//modules/users/pages_permissions_changing_enabled_on_add'
			);
			$this->requireCurrentPassword = (bool) $umiRegistry->getVal('//modules/users/require_current_password');
		...
		public function is_auth() {
			return permissionsCollection::getInstance()->isAuth();
		}
		...
		public function updateUserLastRequestTime($user_id) {
			...
			if ($user_id == UmiCms\Service::SystemUsersPermissions()->getGuestUserId()) {
				return false;
			}
		...
		public function validatePassword($password, $passwordConfirmation = null, $login = false, $public = false) {
			...
			if ($public) {
				...
				$isSecure = self::isPasswordSecure($password);

				if (!$isSecure) {
					$this->errorAddErrors('error-password-not-secure');
					$isValid = false;
				}
		...
		private static function isPasswordSecure($password) {
			return (bool) preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/", $password);
		}
		...
		public function save_perms($ownerId, $mode = 'add') {
			...
			$groupsAreDefined = (is_array($groups) && count($groups) > 0);

			$umiRegistry = regedit::getInstance();
			$guestId = (int) UmiCms\Service::SystemUsersPermissions()->getGuestUserId();
			...
			$defaultGroupId = (int) $umiRegistry->getVal("//modules/users/def_group");
		...
		public static function getRandomPassword($length = 12) {
			...
			while (!self::isPasswordSecure($password)) {
		...
		public function loadUserSettings() {
			...
			$user_id = UmiCms\Service::Auth()->getUserId();
		...
		public function getVariableNamesForMailTemplates() {
			$newRegistrationVariables = [
				'user_id' => getLabel('mail-template-variable-user_id', 'users'),
				'login' => getLabel('mail-template-variable-login', 'users'),
			];

			$restorePasswordVariables = [
				'domain' => getLabel('mail-template-variable-domain', 'users'),
				'restore_link' => getLabel('mail-template-variable-restore_link', 'users'),
				'email' => getLabel('mail-template-variable-email', 'users'),
				'login' => getLabel('mail-template-variable-login', 'users'),
			];

			$userRegisteredVariables = [
				'user_id' => getLabel('mail-template-variable-user_id', 'users'),
				'domain' => getLabel('mail-template-variable-domain', 'users'),
				'login' => getLabel('mail-template-variable-login', 'users'),
				'activate_link' => getLabel('mail-template-variable-activate_link', 'users'),
				'password' => getLabel('mail-template-variable-password', 'users'),
				'lname' => getLabel('mail-template-variable-lname', 'users'),
				'fname' => getLabel('mail-template-variable-fname', 'users'),
				'father_name' => getLabel('mail-template-variable-father_name', 'users'),
			];
			...
			return [
				'users-new-registration-admin-subject' => $newRegistrationVariables,
				'users-new-registration-admin-content' => $newRegistrationVariables,
				'users-restore-password-subject' => $restorePasswordVariables,
				'users-restore-password-content' => $restorePasswordVariables,
				'users-registered-subject' => $userRegisteredVariables,
				'users-registered-content' => $userRegisteredVariables,
				'users-registered-no-activation-subject' => $userRegisteredVariables,
				'users-registered-no-activation-content' => $userRegisteredVariables,
				'users-new-password-subject' => $newPasswordVariables,
				'users-new-password-content' => $newPasswordVariables,
			];
		}
