	...
	class UsersAdmin {
		...
		public function users_list() {
			...
			if (!$umiPermissions->isSv()) {
				$sel->where('id')->notequals(UmiCms\Service::SystemUsersPermissions()->getSvUserId());
			}
		...
		public function groups_list() {
			...
			if (!$umiPermissions->isSv()) {
				$sel->where('id')->notequals(UmiCms\Service::SystemUsersPermissions()->getSvGroupId());
			}
		...
		public function add() {
			...
			if ($mode == "do") {
				...
				$svGroupId = UmiCms\Service::SystemUsersPermissions()
					->getSvGroupId();
		...
		public function del() {
			...
			$defaultGroupId = $umiRegistry->getVal('//modules/users/def_group');
		...
		public function config() {
			...
			if ($mode == 'do') {
				...
				if ($umiNotificationInstalled) {
					$umiRegistry->setVar('//modules/users/use-umiNotifications', $params['config']['boolean:use-umiNotifications']);
				}
			...
			if ($umiNotificationInstalled) {
				$params['config']['boolean:use-umiNotifications'] = $umiRegistry->getVal('//modules/users/use-umiNotifications');
			}

			$params['config']['boolean:require_current_password'] =
				$umiRegistry->getVal('//modules/users/require_current_password');
		...
		public function choose_perms($ownerId = false) {
			...
			if ($ownerId === false) {
				$ownerId = (int) UmiCms\Service::SystemUsersPermissions()
					->getGuestUserId();
			}

			$restrictedModules = ['autoupdate', 'backup'];
