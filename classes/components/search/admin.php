	...
	class SearchAdmin {
		...
		public function config() {
			...
			if ($mode == "do") {
				...
				$config->set("kernel", "search-in-any-part-of-string", $params['config']['boolean:search-in-any-part-of-string']);

				$minWordLength = $params['config']['int:search-min-word-length'];
			...
			$params['config']['boolean:search-in-any-part-of-string'] =  $config->get("kernel", "search-in-any-part-of-string");
			$params['config']['int:search-min-word-length'] =  $config->get("kernel", "search-min-word-length");
			...
			$params['config']['select-multi:search-types'] = $this->module->getSearchTypesOption();
...
SQL;
			...
			foreach ($rows as $row) {
				...
				if ($row['name'] == 'name' && !$row['varchar_val'] && !$row['text_val']){
					$result['link']['content'] = $this->module->searchWithSnippet($row['object_name'] ,$searchString, 'name', 1, 'link');
					$result['text']['content'] = $this->module->searchWithSnippet($row['object_name'] ,$searchString, 'name', 2, 'text');
				} elseif ($row['varchar_val']){
					$result['link']['content'] = $this->module->searchWithSnippet($row['varchar_val'] ,$searchString, $row['name'], 1, 'link');
					$result['text']['content'] = $this->module->searchWithSnippet($row['varchar_val'] ,$searchString, $row['name'], 2, 'text');
				} elseif($row['text_val'] ) {
					$result['link']['content'] = $this->module->searchWithSnippet($row['text_val'] ,$searchString, $row['name'], 1, 'link');
					$result['text']['content'] = $this->module->searchWithSnippet($row['text_val'] ,$searchString, $row['name'], 2, 'text');
				}
